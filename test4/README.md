...
#### 连接自己的账号(new_user_czm)：

![](1.jpg)

#### 创建表，分区以及录入数据，序列的应用，触发器的应用：
...
```sql
select d.*,e.name from departments d,employees e
...
通过本次是实验让我了解到了Oracle表和视图的概念，学习了使用SQL语句Create Table创建表、Select语句插入，修改，删除以及查询数据、使用SQL语句创建视图、部分存储过程和触发器的使用。
...
存储过程的创建： CREATE PROCEDURE 创建的存储过程名字(OUT|IN|INOUT 参数名 数据类型,...,...) 特征 过程体;
